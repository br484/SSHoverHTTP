...
// Config is the configuration for the sshOVERhttp service
type Config struct {
	KeySeed   string
	AuthFile  string
	Auth      string
...
// Server respresent a sshOVERhttp service
type Server struct {
	*cio.Logger
	config       *Config
	fingerprint  string
...
// NewServer creates and returns a new sshOVERhttp server
...
// Run is responsible for starting the sshOVERhttp service.
